...
# ---------- Route ----------
async def search_books_logic(data: dict, db: AsyncIOMotorDatabase):
    try:
        ...
        if not all([user_query, cardnumber, session_id]):
            return JSONResponse(
                content={"error": "Parameters 'query', 'cardnumber', and 'sessionId' are required."},
                status_code=400,
            )

        user_query_clean = clean_query_text(user_query)
        intent = classify_book_intent(user_query_clean)
        logger.info(f"[Intent] Detected intent: {intent} for session '{session_id}' with query: {user_query_clean}")
        ...
        if intent == "isbn_lookup":
            ...
            response_payload = {"type": "isbn", "answer": bot_response}
        # ---------- Book Recommendations ----------
            ...
            response_payload = {"response": [{"type": "recommendation", "answer": bot_response, "books": formatted_books,}]}
        # ---------- General Search ----------
        else:
            ...
            response_payload = {"response": [{"type": "booksearch", "answer": bot_response, "books": formatted_books,}]}
        
        if bot_response: # <<< ADDED
          await save_message_and_get_context(
               db, cardnumber, session_id, "assistant", bot_response
           )
        else:
           logger.error("bot_response was empty, not saving to history.")
...
async def search_books_api(request: Request, db: AsyncIOMotorDatabase = Depends(get_db)):
     """Endpoint to handle book searches. It reads the request and passes it to the logic function."""
